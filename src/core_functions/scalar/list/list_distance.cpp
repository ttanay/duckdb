// What do I need for list_distances_v2
// 1. Type definition
// 2. Return type
...
static void ListDistanceFunction(DataChunk &args, ExpressionState &state, Vector &result) {
	...
	// auto search_l_index = search_l_data.sel->get_index(0); // Q: Why is this needed?
	...
	for (idx_t i = 0; i < count; i++) {
		...
		// While building the selection vector for the search_l, keep moving the selection indices up
		// for (idx_t child_idx = 0; child_idx < l_entry.length; child_idx++) {
		// 	// states vector is full, update
		// 	if (states_idx == STANDARD_VECTOR_SIZE) {
		// 		// update the aggregate state(s)
		// 		Vector slice(l_child, sel_vector, states_idx);
		// 		Vector inputs[] = {slice, search_l_child};
		// 		aggr.function.update(inputs, aggr_input_data, 2, state_vector_update, states_idx);
		...
		// 	auto source_idx = l_child_data.sel->get_index(l_entry.offset + child_idx);
...
ScalarFunction ListDistanceFun::GetFunction() {
	ScalarFunction result({LogicalType::LIST(LogicalType::ANY), LogicalType::LIST(LogicalType::ANY), LogicalType::VARCHAR}, LogicalType::FLOAT, ListDistanceFunction, ListDistanceBind);
